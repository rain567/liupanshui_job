...
def preprocess(data):
    ...
    """
    2.去均值和方差归一化,对于同一个特征，不同的样本中的取值可能会相差非常大
    一些异常小或异常大的数据会误导模型的正确训练；另外，如果数据的分布很分散也会影响训练结果。
    以上两种方式都体现在方差会非常大。此时，我们可以将特征中的值进行标准差标准化，即转换为均值为0，方差为1的正态分布。
    """
    ss = StandardScaler()
...
def main():
    data = pd.read_csv('data30.csv',encoding="ansi")
    #inplace原数据上操作,删除编号列
