...
x_stain_range = np.arange(0, 50, 2, np.float32)
y_oil_range = np.arange(0, 50, 1, np.float32)
...
# 输出为S的规则
rule1 = ctrl.Rule(antecedent=((x_stain['S'] & y_oil['S']) |
                              (x_stain['M'] & y_oil['S'])),
                  consequent=z_time['S'], label='S')

# 输出为M的规则
rule2 = ctrl.Rule(antecedent=((x_stain['L'] & y_oil['S']) |
                              (x_stain['S'] & y_oil['M']) |
                              (x_stain['M'] & y_oil['M']) |
                              (x_stain['L'] & y_oil['M']) |
                              (x_stain['S'] & y_oil['L'])),
...
# 输出为VL的规则
rule3 = ctrl.Rule(antecedent=((x_stain['S'] & y_oil['S']) |
                              (x_stain['L'] & y_oil['L'])),
